...
dictionary = {}
my_list = list(range(len(string)))
str_list = [str(i) for i in my_list]
for key,value in zip(string,str_list) :
...
for key,value in items_purchase.items():
  ...
  items_purchase[key] = new_value

cost = []

for key,value in items_purchase.items():
  cost.append(value)
  cost_int = [int(i) for i in cost]
  total = sum(cost_int)
  if total > wallet :
    cost_int.remove(max(cost_int))
  elif total <= wallet :
    final_cost = [str(i) for i in cost_int]
    for key,value in items_purchase.items():
      if value in final_cost :
        basket.append(key)
        a = set(basket)
        basket = list(a)
